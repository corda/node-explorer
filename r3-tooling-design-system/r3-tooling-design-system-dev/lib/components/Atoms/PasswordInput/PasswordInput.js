...
var PasswordInput = function (_a) {
    ...
    var _c = useState('password'), inputType = _c[0], setInputType = _c[1];
    var passwordId = useState(id ? id : uniqId())[0];
    var _d = useState(false), modified = _d[0], setModified = _d[1];
    useEffect(function () {
        invalid && value === '' ? setModified(true) : setModified(false);
    }, [invalid]);
    var peekPassword = function (event) {
        event.preventDefault();
        inputType === 'password' ? setInputType('text') : setInputType('password');
    };
    return (_jsxs("div", __assign({ className: "input relative " + className }, { children: [_jsxs("label", __assign({ htmlFor: passwordId }, { children: [_jsx(IconCustom, { icon: inputType === 'password' ? 'EyeOutline' : 'EyeOffOutline', className: "input-icon absolute top-3 right-4 h-5-1/2 " + (disabled ? 'text-medium-light-gray-300' : 'text-blue'), onClick: function (event) { return peekPassword(event); } }, void 0), _jsx("input", __assign({ className: "block h-12 w-full rounded-full py-3 px-6 overflow-hidden whitespace-no-wrap tracking-small outline-none border-2 text-sm text-dark-gray  " + (value === ''
                            ? 'border-medium-light-gray-300'
                            : 'border-medium-light-gray') + " focus:border-dark-gray-300 " + (disabled ? 'cursor-not-allowed' : '') + " " + (dark ? 'gray bg-light-gray' : 'white bg-white'), disabled: disabled, id: passwordId, required: required, type: inputType, value: value, "data-invalid": !disabled && invalid, "data-modified": modified, onBlur: function () { return !modified && setModified(true); }, onChange: onChange }, otherProps), void 0), _jsx("span", __assign({ className: "input-label absolute top-sm text-left left-6 bg-transparent font-bold text-sm uppercase tracking-small w-2/3 overflow-hidden whitespace-no-wrap " + (disabled
