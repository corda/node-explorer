...
var TextInput = function (_a) {
    ...
    var textId = useState(id ? id : uniqId())[0];
    var _c = useState(false), modified = _c[0], setModified = _c[1];
    useEffect(function () {
        invalid && value === '' ? setModified(true) : setModified(false);
    }, [invalid]);
    return (_jsxs("div", __assign({ className: "input relative " + className }, { children: [_jsxs("label", __assign({ htmlFor: textId }, { children: [_jsx("input", __assign({ className: "block h-12 w-full rounded-full py-3 px-6 overflow-hidden whitespace-no-wrap outline-none border-2 text-sm tracking-small text-dark-gray " + (value === ''
                            ? 'border-medium-light-gray-300'
                            : 'border-medium-light-gray') + " focus:border-dark-gray-300 " + (disabled ? 'cursor-not-allowed' : '') + " " + (dark ? 'gray bg-light-gray' : 'white bg-white'), disabled: disabled, id: textId, required: required, type: "text", value: value, "data-invalid": !disabled && invalid, "data-modified": modified, onBlur: function () { return !modified && setModified(true); }, onChange: onChange }, otherProps), void 0), _jsx("span", __assign({ className: "input-label absolute top-sm text-left left-6 bg-transparent font-bold text-sm uppercase tracking-small w-2/3 overflow-hidden whitespace-no-wrap " + (disabled
